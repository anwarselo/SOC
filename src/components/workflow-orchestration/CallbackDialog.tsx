...
export function CallbackDialog({ onSave, onCancel }: CallbackDialogProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50">
      <div
        ref={dialogRef}
        className="bg-[#FAFAF8]  p-6 max-w-md w-full mx-4 shadow-2xl border-2 border-[#B8D4D5]"
        role="dialog"
      ...
      >
        ...
        <div className="bg-[#E3F2FD] border-2 border-[#64B5F6]  p-4 mb-4">
        ...
        <div className="mb-6">
          ...
          <input
            ...
            className="w-full px-3 py-2 border-2 border-[#EDEDEA]  focus:outline-none focus:ring-2 focus:ring-[#5F9EA0] focus:border-[#5F9EA0] bg-[#FAFAF8]"
        ...
        <div className="flex gap-2 justify-end">
          <button
            onClick={onCancel}
            className="action-button px-4 py-2 border-2 border-[#B8D4D5]  bg-[#E8F4F5] hover:bg-[#D8E9EA] text-[#2D3436] font-medium transition-all duration-200 hover:shadow-sm"
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={!date}
            className={`px-5 py-2.5  font-medium transition-all duration-200 shadow-sm ${
              date
